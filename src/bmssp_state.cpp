...
static bool add_overflow_u64(uint64_t a, uint64_t b, uint64_t& out) {
  ...
  out = (uint64_t)sum;
  return sum > std::numeric_limits<uint64_t>::max();
#endif
}

static void widen_word(DistWord& w, DistWidth target) {
  if (w.width == target) return;
  if (target == DistWidth::W64 && w.width == DistWidth::W32) {
    w.small.v64 = w.small.v32;
    w.width = DistWidth::W64;
    return;
  }
  if (target == DistWidth::W128) {
    unsigned __int128 val = 0;
    if (w.width == DistWidth::W32)
      val = w.small.v32;
    else if (w.width == DistWidth::W64)
      val = w.small.v64;
    else if (w.width == DistWidth::W128)
      return;
    w.small.v128 = val;
    w.width = DistWidth::W128;
    return;
  }
  ...
  if (du.width == DistWidth::W32 || du.width == DistWidth::W64) {
    ...
    if (base_u <= st.fastpath_u64_max_sum - w) {
      ...
      if (of) {
        ...
        st.overflow_events++;
        st.widen_pairs++;
        st.widen_bytes += (width_bytes(st.dist[su].width) - u_before) +
                          (width_bytes(st.dist[sv].width) - v_before);
#endif
...
#ifdef ENABLE_BIGINT_FALLBACK
      candBig.assign_u128(base128);
      candBig.add_u64_inplace(w);
      kind = CandKind::KBIG;
#else
  ...
  if (dv_was_inf) {
    ...
    if (kind == CandKind::KBIG) {
#ifdef ENABLE_BIGINT_FALLBACK
      dv.big = std::move(candBig);
      dv.width = DistWidth::BIG;
      st.used_bigint = true;
#ifdef ENABLE_BMSSP_VERIFIER
      ...
      if (cand128 <= std::numeric_limits<uint32_t>::max()) {
        dv.width = DistWidth::W32;
        dv.small.v32 = (uint32_t)cand128;
      } else if (cand128 <= std::numeric_limits<uint64_t>::max()) {
        dv.width = DistWidth::W64;
        dv.small.v64 = (uint64_t)cand128;
      } else {
        dv.width = DistWidth::W128;
        dv.small.v128 = cand128;
      }
    }
    st.pred[sv] = u;
    st.hop[sv] = new_hop;
  ...
  if (kind == CandKind::KBIG) {
    ...
    if (dv.width == DistWidth::BIG) {
      ...
      if (dv.width == DistWidth::W32)
        ...
        dv128 = dv.small.v128;
      better = (cand128 < dv128);
      equal_dist = (cand128 == dv128);
    }
