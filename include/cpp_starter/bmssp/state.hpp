...
struct BigInt {
  ...
  static BigInt add(const BigInt& a, const BigInt& b, bool& overflow) {
    BigInt r;
    r.limbs.resize(std::max(a.limbs.size(), b.limbs.size()) + 1, 0);
  ...
  static BigInt add_u64(const BigInt& a, uint64_t b) {
    BigInt r;
    r.limbs.resize(a.limbs.size() + 1, 0);
    unsigned __int128 carry = b;
    std::size_t i = 0;
    for (; i < a.limbs.size(); ++i) {
      unsigned __int128 sum = (unsigned __int128)a.limbs[i] + carry;
      r.limbs[i] = (uint64_t)sum;
      carry = sum >> 64;
    }
    r.limbs[i] = (uint64_t)carry;
    if (r.limbs.back() == 0) r.limbs.pop_back();
    return r;
  }
  void add_u64_inplace(uint64_t b) {
    unsigned __int128 carry = b;
    std::size_t i = 0;
    for (; i < limbs.size() && carry; ++i) {
      unsigned __int128 sum = (unsigned __int128)limbs[i] + carry;
      limbs[i] = (uint64_t)sum;
      carry = sum >> 64;
    }
    if (carry) limbs.push_back((uint64_t)carry);
  }
...
struct DistWord {
  ...
  static DistWord inf() {
    DistWord d;
    d.width = DistWidth::W64;
    d.small.v64 = UINT64_MAX;
    return d;
  }
  bool is_inf() const {
    return width == DistWidth::W64 && small.v64 == UINT64_MAX;
  }
  uint64_t as_u64_clamped() const {
    switch (width) {
      ...
      case DistWidth::W64:
        return small.v64;
      case DistWidth::W128:
        return (uint64_t)small.v128;  // potential truncation for diagnostics only
      case DistWidth::BIG:
        return big.limbs.empty() ? 0 : big.limbs[0];
    }
    return 0;
  }
  static DistWord from_u64(uint64_t v) {
    DistWord d;
    if (v <= std::numeric_limits<uint32_t>::max()) {
      d.width = DistWidth::W32;
      d.small.v32 = (uint32_t)v;
    } else {
      d.width = DistWidth::W64;
      d.small.v64 = v;
    }
  ...
  static DistWord from_u128(unsigned __int128 v) {
    DistWord d;
    if (v <= std::numeric_limits<uint64_t>::max()) {
      d.width = DistWidth::W64;
      d.small.v64 = (uint64_t)v;
    } else {
      d.width = DistWidth::W128;
      d.small.v128 = v;
    }
    return d;
  }
  void assign_u64(uint64_t v) {
    if (v <= std::numeric_limits<uint32_t>::max()) {
      width = DistWidth::W32;
      small.v32 = (uint32_t)v;
    } else {
      width = DistWidth::W64;
      small.v64 = v;
    }
  ...
  void assign_u128(unsigned __int128 v) {
    if (v <= std::numeric_limits<uint64_t>::max())
      assign_u64((uint64_t)v);
    else {
      width = DistWidth::W128;
      small.v128 = v;
    }
  }
};
...
inline BigInt distword_to_big(const DistWord& d) {
  ...
  switch (d.width) {
    case DistWidth::W32:
      b.assign(d.small.v32);
      break;
    case DistWidth::W64:
      b.assign(d.small.v64);
      break;
    case DistWidth::W128:
      b.assign_u128(d.small.v128);
      break;
    case DistWidth::BIG:
      return d.big;
  }
...
inline int compare(const DistWord& a, const DistWord& b) {
  if (a.width == DistWidth::BIG || b.width == DistWidth::BIG) {
    ...
    if (a.width == DistWidth::W32)
      av = a.small.v32;
    else if (a.width == DistWidth::W64)
      av = a.small.v64;
    else if (a.width == DistWidth::W128)
      av = a.small.v128;
    if (b.width == DistWidth::W32)
      bv = b.small.v32;
    else if (b.width == DistWidth::W64)
      bv = b.small.v64;
    else if (b.width == DistWidth::W128)
      bv = b.small.v128;
    if (av < bv) return -1;
    if (av > bv) return 1;
    return 0;
  ...
  switch (a.width) {
    case DistWidth::W32:
      if (a.small.v32 < b.small.v32) return -1;
      if (a.small.v32 > b.small.v32) return 1;
      return 0;
    case DistWidth::W64:
      if (a.small.v64 < b.small.v64) return -1;
      if (a.small.v64 > b.small.v64) return 1;
      return 0;
    case DistWidth::W128:
      if (a.small.v128 < b.small.v128) return -1;
      if (a.small.v128 > b.small.v128) return 1;
      return 0;
    case DistWidth::BIG: /* handled above */
      return 0;
  }
  return 0;
}
...
inline DistState make_state(std::size_t n) {
  ...
  DistWord inf32;
  inf32.width = DistWidth::W32;
  inf32.small.v32 = UINT32_MAX;
  s.dist.assign(n, inf32);
#else
...
inline void mark_complete(DistState& st, int v) {
  if (v >= 0) {
    std::size_t sv = static_cast<std::size_t>(v);
    if (sv < st.complete.size()) st.complete[sv] = 1;
  }
}
...
inline void initialize_sources(DistState& st, const std::vector<int>& S) {
  for (int s : S) {
    ...
    auto& dw = st.dist[ss];
    if (dw.width == DistWidth::W32)
      dw.small.v32 = 0;
    else if (dw.width == DistWidth::W64)
      dw.small.v64 = 0;
    else if (dw.width == DistWidth::W128)
      dw.small.v128 = 0;
    else if (dw.width == DistWidth::BIG)
      dw.big.assign(0);
    st.pred[ss] = s;
    st.hop[ss] = 0;
    st.complete[ss] = 1;
...
inline bool is_complete(const DistState& st, int v) {
  return v >= 0 && static_cast<std::size_t>(v) < st.complete.size() &&
         st.complete[static_cast<std::size_t>(v)] != 0;
}

// Overflow-safe addition with widening; returns true if improved.
bool relax(DistState& st, int u, int v, uint64_t w, bool allow_equal = true,
           bool* widened_out = nullptr);
...
inline void promote_to_big(DistWord& dw) {
  ...
  if (dw.width == DistWidth::W32) {
    dw.big.assign(dw.small.v32);
  } else if (dw.width == DistWidth::W64) {
    dw.big.assign(dw.small.v64);
  } else if (dw.width == DistWidth::W128) {
    dw.big.assign_u128(dw.small.v128);
  }
