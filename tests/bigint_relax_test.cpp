...
TEST(BMSSPBigInt, WidenAndBigIntAdd) {
  // Construct state with two vertices u -> v with large distance near 2^64-1 and weight causing
  // overflow.
  ...
  // Setup vertex 1 as source alternative via vertex 1 -> 0 (same distance path). We simulate by
  // relaxing back with equal distance. Make distance of vertex 1 copied to vertex 0 candidate via
  // weight 0; predecessor should remain 0 since 0 < 1.
  unsigned __int128 dist1_low =
      st.dist[1].width == DistWidth::BIG ? (unsigned __int128)st.dist[1].big.limbs[0] : 0;
  (void)dist1_low;  // placeholder usage in case we later extend assertions.
  widened = false;
  (void)relax(st, 1, 0, 0ULL, true,
              &widened);  // ignore result; only tie-break effect observed via pred
  // Either equal distance relax accepted (then pred[0] becomes 1 only if tie-break decides) or
  // rejected; assert tie-break rule preference for smaller id keeps pred[0]==0.
