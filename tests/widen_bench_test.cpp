...
TEST(BMSSPWidenBench, MeasureWideningOverhead) {
  ...
  // Create a chain to force many relax calls; weights near boundary to cause some widenings when FLAGS differ
  ...
  bool widened=false; (void)widened;
  for(int i=0;i<n-1;++i){
    (void)relax(st, i, i+1, w[static_cast<size_t>(i)], true, &widened);
  }
  // Print counters (visible when running with ctest -V)
  std::cout << "widen_events=" << st.widen_events
            << " overflow_events=" << st.overflow_events
            << " widen_bytes=" << st.widen_bytes
            << " big_bytes=" << st.big_bytes
            << " used_bigint=" << st.used_bigint
            << std::endl;
  SUCCEED();
}
#endif
